...
function buildPolishPrompt(brandTone: string) {
  return `You are a copywriter for Gay Fan Club, a custom merchandise company specializing in hand fans, banners, and promotional items for Pride events, drag shows, and celebrations.
...
export async function POST(request: NextRequest) {
  try {
    ...
    const completion = await openai.chat.completions.create({
      ...
      temperature: 0.7,
      messages: [
        { role: 'system', content: buildPolishPrompt(brandTone) },
